...
\For{$n \gets N_{\min}$ \KwTo $N_{\max}$ \KwStep $\Delta N$}{
  ...
  Print confirmation of generated graph with $n$ nodes and $n \cdot E$ edges\;
}
\end{algorithm} 
\vspace{1.5em}
...
When processing large-scale graphs memory exhaustion and application crashes are inevitable. Therefore, error handling strategies are required to ensure that experiments remain reproducible and manageable. For simplicity reasons, in this thesis, it is only differentiated between Out-of-Memory (OOM) failures and timeouts. \par
The Java Out-of-Memory exception is a very common scenario and expected in this experimental setup since the goal is to strain memory. It is usually thrown when there is not enough memory in the Java heap to allocate for an object. This means the Garbage Collector is not able to free up sufficient space to accommodate an object.  
